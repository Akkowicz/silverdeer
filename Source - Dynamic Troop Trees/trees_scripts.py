	...
	[
		...
		(try_begin),
			...
			(else_try),
				...
				(assign, reg21, CE_PREREQ_HIGH_UPKEEP_PENALTY),
				(str_store_string, s21, "@{s21}  (+{reg21}% Wages)"),
			(else_try),
				(eq, ":prereq", PREREQ_EXPENSIVE),
				(assign, reg21, CE_PREREQ_EXPENSIVE_PENALTY),
				(str_store_string, s21, "@{s21}  (+{reg21}% Hiring Cost)"),
			(try_end), 
	...
	[
		...
		(try_begin),
			...
			(else_try),
				...
				(assign, reg21, CE_ABILITY_DEVOTED_DISCOUNT),
				(str_store_string, s21, "@{s21}  (-{reg21}% Troop Wages)"),
			(else_try),
